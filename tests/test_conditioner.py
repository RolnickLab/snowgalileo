...
class TestConditioner(unittest.TestCase):
    ...
    def test_end_to_end_grads_nonzero_lora(self):
        backbone_depth = 2
        ...
        for channel in range(2):
            for depth in range(backbone_depth):
                for param in encoder.conditioner.param_types:
                    self.assertTrue(
                        encoder.conditioner.loras[f"{channel}_{depth}_{param}_a"].grad is not None
                    )
                    self.assertTrue(
                        encoder.conditioner.loras[f"{channel}_{depth}_{param}_b"].grad is not None
                    )
        for channel in range(2, len(UNMASKING_CHANNEL_GROUPS)):
            for depth in range(backbone_depth):
                for param in encoder.conditioner.param_types:
                    self.assertTrue(
                        encoder.conditioner.loras[f"{channel}_{depth}_{param}_a"].grad is None
                    )
                    self.assertTrue(
