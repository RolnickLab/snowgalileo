...
class TestMasking(unittest.TestCase):
    def test_forward_filling_masked_data_per_channel_else_median(self):
        ...
        four_dim_data_c1 = torch.tensor([[[[1.0, float('nan'), float('nan'), 4.0, 5.0]]]])  # shape (B=1, S=1, C=1, T=5)
        four_dim_data_c2 = torch.tensor([[[[float('nan'), float('nan'), 3.0, float('nan'), 5.0]]]])
        four_dim_data_c3 = torch.tensor([[[[float('nan'), float('nan'), float('nan'), float('nan'), float('nan')]]]])
        ...
        four_dim_data = torch.cat([four_dim_data_c1, four_dim_data_c2, four_dim_data_c3], dim=2)  # shape (B=1, S=1, C=3, T=5)
        four_dim_mask = torch.where(four_dim_data == float('nan'), 1, 0)
        four_dim_time = torch.zeros_like(four_dim_data)

        three_dim_data = torch.cat([three_dim_data_c1, three_dim_data_c2, three_dim_data_c3], dim=2)  # shape (B=1, S=1, C=3)
        three_dim_mask = torch.where(three_dim_data == float('nan'), 1, 0)
        three_dim_time = torch.zeros_like(three_dim_data)

        filled_four_dim_data, _ = forward_filling_masked_data_per_channel_else_median(four_dim_data, four_dim_mask, four_dim_time)
        filled_three_dim_data, _ = forward_filling_masked_data_per_channel_else_median(three_dim_data, three_dim_mask, three_dim_time)
