...
2. **Spatial Bounding Box (`EEBoundingBox`)**:
   ...
   - Degree conversions are dynamically computed based on the coordinate's latitude using local Earth radius estimates:
     $$m_{\text{lat}} \approx 111132.954 - 559.822\cos(2\phi) + 1.175\cos(4\phi)$$
     $$m_{\text{lon}} \approx 111412.84\cos(\phi) - 93.5\cos(3\phi) + 0.118\cos(5\phi)$$
...
2. **Temporal Combination**:
   ...
   $$\text{[band\_1\_t0, band\_2\_t0, ..., band\_C\_t0, band\_1\_t1, ..., band\_C\_t7]}$$
...
5. **Downsampling via Block Pooling**:
   ...
   - **Medium-Res** is average-pooled over $20 \times 20$ pixel blocks (`h_block = H // new_H` where $H=100$, $new\_H=5$) to yield **$5 \times 5 \times 8 \times 2$**.
