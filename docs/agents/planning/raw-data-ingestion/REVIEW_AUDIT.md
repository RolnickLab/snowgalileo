...
| :- | :- | :- | :- |
| 1 | DEM slope/aspect 4326 distortion | **Partially valid — but the proposed UTM fix is wrong** | `copernicus_dem.py:14-16` calls `ee.Terrain.slope/aspect` on the DEM's **native projection**; GEE's terrain ops are latitude-aware (true metric pixel spacing), then export resamples to 4326/scale=10. The parity target therefore never computes in EPSG:32611. The review's "compute in 32611, reproject to 4326" detour would **diverge from the GEE reference patches** AC-21 tests against. Real defect = loose FR-15 wording, not the algorithm. **Fix: tighten FR-15 to "latitude-correct metric pixel spacing matching `ee.Terrain`," not a UTM detour.** |
| 2 | **CRITICAL:** Landsat 32612 is a hardcode bug; AOI is UTM 11 | **FALSE** | `gdalinfo` on `LC09_L1TP_042024_20250310_..._B4.TIF` → `ID["EPSG",32612]` (UTM 12N). USGS delivers WRS-2 path/row scenes in their assigned UTM zone regardless of AOI longitude. **All** archive Landsat scenes (paths 042–044) are natively 32612 (already recorded `DATA_ANALYSIS.md:545-546`). Clipping in native 32612 is correct + non-destructive; the adapter reprojects 32612→4326 later (FR-5). **No change; optional defensive "query CRS" note added.** |
