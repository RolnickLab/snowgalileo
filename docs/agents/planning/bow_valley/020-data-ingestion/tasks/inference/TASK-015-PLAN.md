...
- `run() -> list[Path]`: for each `day in inference_days(...)`:
  ...
  - Batch cells (`batch_size`), run `model(*batched, patch_size_high_res=10, ...)`,
    reshape each row → `(10,10)`; map `cell_id -> fsc`, with `None` when
...
Entry-point scripts (`../../../../../../../scripts/developer_scripts/bow_valley_inference_local/04_infer_bow_valley_daily_fsc.py`), checkpoint/model build,
