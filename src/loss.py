...
def construct_target_encoder_masks(
    ...
    if method == "decoder_only":
        ...
        return ~(s_t_h_m == 2), ~(sp_m == 2), ~(t_m == 2), ~(st_m == 2)
    elif method == "all":
        # we want all zeros
        return (
        ...
        return s_t_h_m == 1, sp_m == 1, t_m == 1, st_m == 1
    else:
...
def mse_loss(
    ...
    expanded_s_t_m = repeat(s_t_h_m, "b h w t c_g -> b h w t c_g d", d=encoder_size)
...
def seq_and_cat(s_t_h, sp, t, st):
    ...
    return torch.cat([s_t_h, sp, t, st], dim=1)
...
def patch_disc_loss(
    ...
    all_preds = seq_and_cat(p_s_t_h, p_sp, p_t, p_st)
    all_targets = seq_and_cat(t_s_t_h, t_sp, t_t, t_st)
