...
:class:`~snow_galileo.data.local_sources.exporter.LocalSourceExporter` in an initializer and reuses
...
def export_cells_parallel(
    *,
    ...
    Args:
        ...
        verify_s1_cache: Have each worker's exporter **verify** (not build) that the
            offline per-granule S1 SNAP cache covers each cell's window, failing loud if a
