...
satisfies the :class:`~snow_galileo.data.local_sources.base.LocalSourceAdapter` port but
returns :func:`~snow_galileo.data.local_sources.base.create_placeholder` (an all-``-9999``
...
lives in :mod:`snow_galileo.data.local_sources.layout`, never retyped here.
...
from snow_galileo.data.local_sources.layout import DYNAMIC_BANDS, STATIC_BANDS
...
def dynamic_adapters() -> list[PlaceholderAdapter]:
    ...
    :data:`~snow_galileo.data.local_sources.layout.DYNAMIC_BANDS` (asserted below), so the
...
def static_adapters() -> list[PlaceholderAdapter]:
    ...
    ``bands_out`` is exactly :data:`~snow_galileo.data.local_sources.layout.STATIC_BANDS`
    (asserted below).
