...
:class:`~snow_galileo.data.local_sources.base.LocalSourceAdapter` *port*, never on a
...
:mod:`snow_galileo.data.local_sources.layout` and are never retyped here.
"""
...
from snow_galileo.data.config import DAYS_PER_TIMESTEP, NO_DATA_VALUE, NUM_TIMESTEPS
...
from snow_galileo.data.local_sources.placeholder import (
    PlaceholderAdapter,
    dynamic_adapters,
    static_adapters,
)
...
class LocalSourceExporter:
    ...
    def _build_dynamic_adapters(self) -> list[LocalSourceAdapter]:
        ...
        from snow_galileo.data.local_sources.modis import ModisAdapter, ModisCloudAdapter
        from snow_galileo.data.local_sources.s1 import S1Adapter
        from snow_galileo.data.local_sources.s2 import S2Adapter, S2CloudAdapter
        from snow_galileo.data.local_sources.s3 import S3Adapter
        from snow_galileo.data.local_sources.viirs import ViirsCoarseAdapter, ViirsFineAdapter
    ...
    def _build_static_adapters(self) -> list[LocalSourceAdapter]:
        ...
        is the real :class:`~snow_galileo.data.local_sources.worldcover.WorldCoverAdapter`
        (TASK-006), and ``DEM``/``slope``/``aspect`` are served by the single
        :class:`~snow_galileo.data.local_sources.dem.DemAdapter` (TASK-007), which emits all
    ...
    def _ensure_s1_cache(self, cell: GridCell, window_end: datetime.date) -> None:
        ...
        if self.placeholder or not self.verify_s1_cache:
            return
        from snow_galileo.data.local_sources.s1_snap import ensure_s1_cache
