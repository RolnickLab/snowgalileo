...
class EvalTask(ABC):
    ...
    def group_encodings_per_token(
        self, model, s_t_h_x, sp_x, t_x, st_x, s_t_h_m, sp_m, t_m, st_m
    ) -> torch.Tensor:
        encodings = rearrange(
            model.apply_mask_and_average_tokens_per_patch(
                s_t_h_x, sp_x, t_x, st_x, s_t_h_m, sp_m, t_m, st_m
            ),
            "b n_t n_f -> (b n_t) n_f",
        )
        return encodings
    ...
    def train_sklearn_model(
        ...
        for masked_output, label in tqdm(train_dl, desc="Computing encodings for sklearn"):
            ...
            with torch.no_grad():
                s_t_h_x, sp_x, t_x, st_x, s_t_h_m, sp_m, t_m, st_m, _ = pretrained_model(
                    s_t_h_x=s_t_h_x,
                    sp_x=sp_x,
                    t_x=t_x,
                    st_x=st_x,
                ...
                if self.spatial_token_prediction:
                    encodings = (
                        self.group_encodings_per_token(
                            pretrained_model, s_t_h_x, sp_x, t_x, st_x, s_t_h_m, sp_m, t_m, st_m
                        )
                        .cpu()
                        .numpy()
                    )
