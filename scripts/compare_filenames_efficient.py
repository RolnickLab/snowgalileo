...
argparser.add_argument(
    "--input_folder", type=str, default="landsat_eval_tifs/patches_UTM_5_95_cropped/train"
)
argparser.add_argument(
    "--output_folder", type=str, default="landsat_eval_masks/all/patches_UTM_5_95_subset/train"
)
...
def compare_filenames(folder1, folder2):
    ...
    print(
        f"Total non-matching (unique across both): {len(only_in_folder1) + len(only_in_folder2)}"
    )
    ...
    for prefix in list(common_prefixes)[:10]:
        print(
            f"Prefix: {prefix}, File in {folder1}: {prefixes1[prefix]}, File in {folder2}: {prefixes2[prefix]}"
        )
