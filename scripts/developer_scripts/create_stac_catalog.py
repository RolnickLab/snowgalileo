    ...
    if tile_name not in sentinel2_bounds_cache:
        try:
            with zipfile.ZipFile(zip_path, "r") as z:
                mtd_name = [x for x in z.namelist() if "MTD_MSIL1C.xml" in x or "MTD_MSIL2A.xml" in x][0]
                ...
                if fp:
                    ...
                    lats = coords[0::2]
                    lons = coords[1::2]
                    sentinel2_bounds_cache[tile_name] = [min(lons), min(lats), max(lons), max(lats)]
        except Exception as e:
...
def build_catalog():
    ...
    catalog = Catalog(id="bow-valley-selection-raw", description="STAC catalog for Bow Valley Selection raw geospatial datasets.")
    ...
    collections_metadata = {
        "dem": {
            ...
            "temporal": [datetime.datetime(2011, 2, 6, tzinfo=datetime.timezone.utc), datetime.datetime(2014, 8, 27, tzinfo=datetime.timezone.utc)]
        },
        "worldcover": {
            "title": "ESA WorldCover 10m 2021",
            "description": "ESA WorldCover land cover classification product.",
            "spatial": [-117.0, 48.0, -114.0, 51.0],
            "temporal": [datetime.datetime(2021, 1, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2021, 12, 31, tzinfo=datetime.timezone.utc)]
        },
        "era5": {
            ...
            "temporal": [datetime.datetime(2025, 3, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2025, 3, 31, tzinfo=datetime.timezone.utc)]
        },
        "landsat8": {
            ...
            "temporal": [datetime.datetime(2025, 3, 1, tzinfo=datetime.timezone.utc), datetime.datetime(2025, 5, 31, tzinfo=datetime.timezone.utc)]
        },
        "landsat9": {
        ...
        if col_id == "dem":
            ...
            for root, dirs, files in os.walk(col_dir):
                for f in files:
                    if f.endswith("_DEM.tif"):
                        ...
                        item.add_asset(key="data", asset=Asset(href=str(rel_path), media_type=pystac.MediaType.GEOTIFF))
            ...
            for root, dirs, files in os.walk(col_dir):
                for f in files:
                    if f.endswith(".nc"):
                        ...
                        item.add_asset(key="data", asset=Asset(href=str(rel_path), media_type="application/x-netcdf"))
                        collection.add_item(item)
            ...
            for f in os.listdir(col_dir):
                if f.endswith(".tar"):
                    ...
                    item.add_asset(key="archive", asset=Asset(href=str(rel_path), media_type="application/x-tar"))
                    collection.add_item(item)
            ...
            for f in os.listdir(col_dir):
                if f.endswith(".hdf") or f.endswith(".h5"):
                    ...
                    dt = datetime.datetime.strptime(julian_str, "%Y%j").replace(tzinfo=datetime.timezone.utc)
                    ...
                    item.add_asset(key="data", asset=Asset(href=str(rel_path), media_type="application/x-hdf"))
                    collection.add_item(item)
            ...
            for f in os.listdir(col_dir):
                if f.endswith(".zip"):
                    ...
                    item.add_asset(key="archive", asset=Asset(href=str(rel_path), media_type="application/zip"))
                    collection.add_item(item)
