...
argparser.add_argument(
    "--csv", type=str, default="evaluation_results_attn_snow_with_high_res.pth.csv"
)
argparser.add_argument(
    "--csv_compare", type=str, default="evaluation_results_attn_snow_no_high_res.pth.csv"
)
...
def compare_csv_results(hr_csv: Path, no_hr_csv: Path, metric: str = "rmse"):
    ...
    assert len(df_hr) == len(df_no_hr), (
        "CSV files must have the same number of rows for comparison."
    )
    assert df_hr.columns.equals(df_no_hr.columns), (
        "CSV files must have the same columns for comparison."
    )
    assert all(df_hr["filename"] == df_no_hr["filename"]), (
        "CSV files must have the same filenames in the same order."
    )
    ...
    comparison_df[f"{metric}_diff"] = (
        comparison_df[f"{metric}_with_hr"] - comparison_df[f"{metric}_without_hr"]
    )

    # get the number of cases where excluding high-res improved performance
    improved_cases = (comparison_df[f"{metric}_diff"] > 0).sum()
    print(
        f"Number of cases where excluding high-res improved performance: {improved_cases} out of {len(comparison_df)}"
    )

    # get the number of cases where excluding high-res worsened performance
    worsened_cases = (comparison_df[f"{metric}_diff"] < 0).sum()
    print(
        f"Number of cases where excluding high-res worsened performance: {worsened_cases} out of {len(comparison_df)}"
    )
